 ...
 */
public class AgentMontecarlo {
	final String name = "monte";
	
	Card strategy(Sevens sevens) {
		...
		for (Card card : playableAndHold) {
			...
			for (int loop = 0 ; loop < M / playableAndHold.size() ; loop++) {
				...
				simSevens.setupSevens(sevens.players, sevens.deck, sevens.layout, (sevens.turn + 1) % sevens.turn, sevens.totalTurn + 1, MyUtil.SIM, sevens.playersOrder, sevens.history, agents);
				for (int index = 0 ; index < sevens.players.size() ; index++) {
					player = simSevens.players.get(index);
					if (sevens.turn != index) {
						cards = cards.getIntersection(player.hand);
					}
				}
				cards.shuffle();
				for (int index = 0 ; index < sevens.players.size() ; index++) {
